...
## Known issues
- **Resolved during Lazar's first Xcode build (D-012):** the first `⌘B` reported `Multiple commands produce …/ECHO.app/.gitkeep` (1 error + 2 "duplicate output file" warnings). Cause: Xcode's file-system synchronized groups copy `.gitkeep` into the app bundle as a resource, and the three identically-named `.gitkeep` files under `ECHO/` (Models, Audio, Haptics) collided. Fixed by removing those three `.gitkeep` files. **Lazar must Clean Build Folder (⇧⌘K) and rebuild** to clear the stale duplicate-output entries from DerivedData. (The Phase 1.01 report's assumption that synchronized groups ignore dotfiles was wrong.)
- Apart from that fix, the build still has not been validated by a successful Xcode compile / Simulator run in the Code environment (no full Xcode here). Lazar's rebuild is the first real confirmation; if Xcode reports any further issue, fix it there and note it.
